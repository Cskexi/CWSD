        ...
        <el-Table :data="TableData" style="width: 100%">
            ...
            <el-Table-column label="操作" fixed="right">
                <template slot-scope="scope">
                    <el-button v-if="scope.row.status===0" size="mini" type="danger" @click="changeStatus(scope.row, 1)"
                        style="margin-bottom: 10px;">付款</el-button>
                    <el-button v-if="scope.row.status===2"  size="mini" type="danger" @click="changeStatus(scope.row, 3)"
                        style="margin-bottom: 10px;">收货</el-button><br>
                    <el-button  v-if="scope.row.status===3" size="mini" type="danger" @click="edit(scope.row,4)"
                        style="margin-bottom: 10px;">评价</el-button>
                    <el-button  v-if="scope.row.status===0||scope.row.status===1" size="mini" type="danger" @click="changeStatus(scope.row, -1)">取消订单</el-button><br>
                    <el-button v-if="scope.row.status===2||scope.row.status===3" size="mini" type="danger" @click="edit(scope.row,-2)" style="margin-bottom: 10px;">申请退单</el-button><br>
                </template>
            </el-Table-column>
        </el-Table>
